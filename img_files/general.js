...
if(window.location.hash)
{
	if(window.location.hash != '#_=_')
	{
		...
		var param = "src="+srce+"&pageref="+encodeURIComponent(page_ref);
		new Ajax.Request(
			url,
			{
				method:'get',
...
function exclusiveSelect(which)
{
	...
	if(choose == 6 || choose == 7) {
		jQuery('#searchform2').attr('action', '/search.php');
		jQuery('#searchform2').attr('method', 'get');
		jQuery('#searchform').attr('action', '/search.php');
		jQuery('#searchform').attr('method', 'get');
	} else {
		jQuery('#searchform2').attr('action', '');
		jQuery('#searchform2').attr('method', 'post');
	...
	for(var i = 0;i<filetypes.length; i++)
	{
		...
		else document.getElementById(idname).checked = true;
	}
	...
	for(var i = 0;i<filetypes.length; i++)
	{
		...
		if(document.getElementById(idname).checked == true)
		{
			document.getElementById(elemname).style.fontWeight = "600";
			document.getElementById(elemname).style.backgroundColor = "#e0e0e0";
...
jQuery( document ).ready(function() {
    ...
    jQuery('#search_open').click(function(){
        ...
        var strLength = jQuery('#searchtext').val().length * 2;
        jQuery('#searchtext').focus();
        jQuery('#searchtext')[0].setSelectionRange(strLength, strLength);
    });
