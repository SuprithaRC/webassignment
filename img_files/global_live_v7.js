...
document.addEventListener("DOMContentLoaded", function(event) {
    if(typeof is_signform_preload !== 'undefined' && is_signform_preload == true){
        jQuery.colorbox({
            iframe:true,
            innerWidth:550,
            innerHeight:signform_preload_height,
            fastIframe:false,
            href:site_https+"/"+signform_preload_href
        });
    }
    ...
    /*jQuery(".signup_panel_btn").colorbox({
        ...
        fastIframe:false,
        href:site_https+"/member_iframe.php?signform=signup",
        onOpen:function(){
            cboxMemberLoading = 1;
    ...
    jQuery(".signin_panel_btn").colorbox({
        ...
        innerHeight:signform_height,
        fastIframe:false,
        href:site_https+"/member_iframe.php?signform=signin",
        onOpen:function(){
            cboxMemberLoading = 1;
            cboxMemberTimer = setTimeout(function(){
                if(cboxMemberLoading){
                    window.location = site_https+"/login.php";
                }
            }, cboxMemberTimeout);
        },
        onComplete: function(){
    ...
    // signup and login button onclick to direct to the respective pages
    jQuery(".signup_panel_btn").click(function (){
        window.location = site_https+"/register.php";
    });

    jQuery(".signin_panel_btn").click(function (){
        window.location = site_https+"/login.php";
    });
});
